...
const I18N = {
  zh: {
    ...
    how_nback:`规则：方块一个个亮起。当【当前位置】与【N 步之前那次】相同 → 点「🟰匹配」（空格键）。右上角 🔊 打开后是双通道模式：每回合还会读一个字母，字母与 N 步前的字母相同 → 点「🔤字母」（K 键）。两条流独立计分，都要达标才通关。N1~N6 即难度。每回合 3 秒。`,
    nb_letter:"字母", nb_cue:"声音/双通道", nb_dual_on:"双通道模式", nb_dual_off:"纯视觉模式",
  ...
  en: {
    ...
    how_nback:`How to play: tiles light up one by one. When the CURRENT position matches the one N steps earlier, tap Match (or Space). With the speaker icon ON (top-right), it becomes dual n-back: a letter is also spoken each turn — if it matches the letter from N steps back, tap the Letter button (K key). Both streams are scored separately and both must pass. N1–N6 = difficulty. 3 seconds per turn.`,
    nb_letter:"Letter", nb_cue:"Voice / dual channel", nb_dual_on:"dual mode", nb_dual_off:"visual only",
