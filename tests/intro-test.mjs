...
const proc=spawn(HS,["--headless","--no-sandbox","--disable-gpu","--remote-debugging-port=9230","about:blank"],{stdio:"ignore"});
for(let i=0;i<75;i++){ try{ if((await fetch("http://127.0.0.1:9230/json")).ok) break; }catch{} await sleep(200); }
const t=await (await fetch("http://127.0.0.1:9230/json")).json();
...
await send("Page.navigate",{url:"https://focusplay.516278.xyz/index.html"}); await sleep(2500);
